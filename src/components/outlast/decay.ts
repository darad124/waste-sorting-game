 ...
 *
 *  Ported from tools/outlast/decay.js.
 * ====================================================================== */
...
export function laneDefs(p: string, item: OutlastItem): string {
...
export function laneOverlays(p: string, item: OutlastItem): string {
  const m = item.material;
  if (m === "paper" || m === "organic")
...
export function laneMicro(p: string, item: OutlastItem): string {
...
export function bodyAttrs(p: string, item: OutlastItem): string {
...
const g = (id: string) => document.getElementById(id) as unknown as SVGElement & HTMLElement;

/** Advance one lane to decay `d`, 0 intact, 1 finished. */
export function apply(p: string, item: OutlastItem, d: number): void {
  const m = item.material;
  const body = g(p + "body");
...
export function decayAt(logT: number, years: number): number {
